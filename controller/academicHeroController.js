const AcademicHero = require('../models/academicHero');
const fs = require('fs');
...
exports.getActive = async (req, res) => {
    try {
        const heroData = await AcademicHero.findOne({ status: 'active' });
...
exports.update = async (req, res) => {
    try {
        ...
        let heroSection = await AcademicHero.findOne({ status: 'active' });
        ...
        if (!heroSection) {
            ...
            heroSection = new AcademicHero({
                title,
                description,
                buttonText: buttonText || "Read More",
                buttonLink: buttonLink || "/academic",
                imageUrl: '/uploads/academic-hero/' + req.file.filename
            });
        } else {
            ...
            if (req.file) {
                ...
                heroSection.imageUrl = '/uploads/academic-hero/' + req.file.filename;
            }
...
exports.delete = async (req, res) => {
    try {
        const heroSection = await AcademicHero.findById(req.params.id);
        if (!heroSection) {
